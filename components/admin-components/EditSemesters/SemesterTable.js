...
//API
import { deleteSemester, addNewSemester, getSemesters } from "../../../api/APIHelper";
...
const SemesterTable = ({
  ...
  //Handle remove
  const handleRemoveSemester = async (term, year) => {
    try {
      {
    ...
    <MaterialTable
      ...
      editable={{
        ...
        onRowDelete: (oldData) =>
          new Promise((resolve, reject) => {
            setTimeout(() => {
              handleRemoveSemester(oldData.term, oldData.year);
              resolve();
            }, 1000);
          }),
      }}
